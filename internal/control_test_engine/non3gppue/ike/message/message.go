...
func Encode(ikeMessage *IKEMessage) ([]byte, error) {
	...
	if ikeMessage != nil {
		log.Info("[IKE] Start encoding IKE message")
		...
		log.Tracef("Encoded %d bytes", len(ikeMessageData))
		log.Tracef("IKE message data:\n%s", hex.Dump(ikeMessageData))
...
func Decode(rawData []byte) (*IKEMessage, error) {
	...
	log.Tracef("Received IKE message:\n%s", hex.Dump(rawData))
...
func DecodePayload(nextPayload uint8, rawData []byte) ([]IKEPayloadType, error) {
	...
	for len(rawData) > 0 {
		// bounds checking
		log.Trace("[IKE] DecodePayload(): Decode 1 payload")
		if len(rawData) < 4 {
...
func (securityAssociation *SecurityAssociation) marshal() ([]byte, error) {
	log.Info("[IKE][SecurityAssociation] marshal(): Start marshalling")
...
func (securityAssociation *SecurityAssociation) unmarshal(rawData []byte) error {
	log.Info("[IKE][SecurityAssociation] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][SecurityAssociation] unmarshal(): Payload length %d bytes", len(rawData))

	for len(rawData) > 0 {
		log.Trace("[IKE][SecurityAssociation] unmarshal(): Unmarshal 1 proposal")
		// bounds checking
		if len(rawData) < 8 {
		...
		if rawData[0] == 0 {
			log.Trace("[IKE][SecurityAssociation] This proposal is the last")
		}
		// Log the number of transform in the proposal
		log.Tracef("[IKE][SecurityAssociation] This proposal contained %d transform", rawData[7])
		...
		for len(transformData) > 0 {
			// bounds checking
			log.Trace("[IKE][SecurityAssociation] unmarshal(): Unmarshal 1 transform")
			...
			if transformData[0] == 0 {
				log.Trace("[IKE][SecurityAssociation] This transform is the last")
			}
...
func (keyExchange *KeyExchange) marshal() ([]byte, error) {
	log.Info("[IKE][KeyExchange] marshal(): Start marshalling")
...
func (keyExchange *KeyExchange) unmarshal(rawData []byte) error {
	log.Info("[IKE][KeyExchange] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][KeyExchange] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][KeyExchange] unmarshal(): Unmarshal 1 key exchange data")
		// bounds checking
		if len(rawData) <= 4 {
...
func (identification *IdentificationInitiator) marshal() ([]byte, error) {
	log.Info("[IKE][Identification] marshal(): Start marshalling")
...
func (identification *IdentificationInitiator) unmarshal(rawData []byte) error {
	log.Info("[IKE][Identification] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Identification] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Identification] unmarshal(): Unmarshal 1 identification")
		// bounds checking
		if len(rawData) <= 4 {
...
func (certificate *Certificate) marshal() ([]byte, error) {
	log.Info("[IKE][Certificate] marshal(): Start marshalling")
...
func (certificate *Certificate) unmarshal(rawData []byte) error {
	log.Info("[IKE][Certificate] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Certificate] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Certificate] unmarshal(): Unmarshal 1 certificate")
		// bounds checking
		if len(rawData) <= 1 {
...
func (certificateRequest *CertificateRequest) marshal() ([]byte, error) {
	log.Info("[IKE][CertificateRequest] marshal(): Start marshalling")
...
func (certificateRequest *CertificateRequest) unmarshal(rawData []byte) error {
	log.Info("[IKE][CertificateRequest] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][CertificateRequest] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][CertificateRequest] unmarshal(): Unmarshal 1 certificate request")
		// bounds checking
		if len(rawData) <= 1 {
...
func (authentication *Authentication) marshal() ([]byte, error) {
	log.Info("[IKE][Authentication] marshal(): Start marshalling")
...
func (authentication *Authentication) unmarshal(rawData []byte) error {
	log.Info("[IKE][Authentication] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Authentication] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Authentication] unmarshal(): Unmarshal 1 authentication")
		// bounds checking
		if len(rawData) <= 4 {
...
func (nonce *Nonce) marshal() ([]byte, error) {
	log.Info("[IKE][Nonce] marshal(): Start marshalling")
...
func (nonce *Nonce) unmarshal(rawData []byte) error {
	log.Info("[IKE][Nonce] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Nonce] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Nonce] unmarshal(): Unmarshal 1 nonce")
...
func (notification *Notification) marshal() ([]byte, error) {
	log.Info("[IKE][Notification] marshal(): Start marshalling")
...
func (notification *Notification) unmarshal(rawData []byte) error {
	log.Info("[IKE][Notification] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Notification] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Notification] unmarshal(): Unmarshal 1 notification")
		// bounds checking
		if len(rawData) < 4 {
...
func (delete *Delete) marshal() ([]byte, error) {
	log.Info("[IKE][Delete] marshal(): Start marshalling")
...
func (delete *Delete) unmarshal(rawData []byte) error {
	log.Info("[IKE][Delete] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Delete] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Delete] unmarshal(): Unmarshal 1 delete")
		// bounds checking
		if len(rawData) <= 4 {
...
func (vendorID *VendorID) marshal() ([]byte, error) {
	log.Info("[IKE][VendorID] marshal(): Start marshalling")
	return vendorID.VendorIDData, nil
}

func (vendorID *VendorID) unmarshal(rawData []byte) error {
	log.Info("[IKE][VendorID] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][VendorID] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][VendorID] unmarshal(): Unmarshal 1 vendor ID")
...
func (trafficSelector *TrafficSelectorInitiator) marshal() ([]byte, error) {
	log.Info("[IKE][TrafficSelector] marshal(): Start marshalling")
		...
		for _, individualTrafficSelector := range trafficSelector.TrafficSelectors {
			if individualTrafficSelector.TSType == TS_IPV4_ADDR_RANGE {
				...
				if len(individualTrafficSelector.StartAddress) != 4 {
					log.Errorf("Address length %d", len(individualTrafficSelector.StartAddress))
...
func (trafficSelector *TrafficSelectorInitiator) unmarshal(rawData []byte) error {
	log.Info("[IKE][TrafficSelector] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][TrafficSelector] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][TrafficSelector] unmarshal(): Unmarshal 1 traffic selector")
		// bounds checking
		if len(rawData) < 4 {
...
func (encrypted *Encrypted) marshal() ([]byte, error) {
	log.Info("[IKE][Encrypted] marshal(): Start marshalling")

	if len(encrypted.EncryptedData) == 0 {
		log.Warn("[IKE][Encrypted] The encrypted data is empty")
	}
...
func (encrypted *Encrypted) unmarshal(rawData []byte) error {
	log.Info("[IKE][Encrypted] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Encrypted] unmarshal(): Payload length %d bytes", len(rawData))
...
func (configuration *Configuration) marshal() ([]byte, error) {
	log.Info("[IKE][Configuration] marshal(): Start marshalling")
...
func (configuration *Configuration) unmarshal(rawData []byte) error {
	log.Info("[IKE][Configuration] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][Configuration] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][Configuration] unmarshal(): Unmarshal 1 configuration")
		// bounds checking
		if len(rawData) <= 4 {
		...
		for len(configurationAttributeData) > 0 {
			log.Trace("[IKE][Configuration] unmarshal(): Unmarshal 1 configuration attribute")
			// bounds checking
...
func (eap *EAP) marshal() ([]byte, error) {
	log.Info("[IKE][EAP] marshal(): Start marshalling")
...
func (eap *EAP) unmarshal(rawData []byte) error {
	log.Info("[IKE][EAP] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][EAP] unmarshal(): Payload length %d bytes", len(rawData))

	if len(rawData) > 0 {
		log.Trace("[IKE][EAP] unmarshal(): Unmarshal 1 EAP")
		// bounds checking
		if len(rawData) < 4 {
...
func (eapIdentity *EAPIdentity) marshal() ([]byte, error) {
	log.Info("[IKE][EAP][Identity] marshal(): Start marshalling")
...
func (eapIdentity *EAPIdentity) unmarshal(rawData []byte) error {
	log.Info("[IKE][EAP][Identity] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][EAP][Identity] unmarshal(): Payload length %d bytes", len(rawData))
...
func (eapNotification *EAPNotification) marshal() ([]byte, error) {
	log.Info("[IKE][EAP][Notification] marshal(): Start marshalling")
...
func (eapNotification *EAPNotification) unmarshal(rawData []byte) error {
	log.Info("[IKE][EAP][Notification] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][EAP][Notification] unmarshal(): Payload length %d bytes", len(rawData))
...
func (eapNak *EAPNak) marshal() ([]byte, error) {
	log.Info("[IKE][EAP][Nak] marshal(): Start marshalling")
...
func (eapNak *EAPNak) unmarshal(rawData []byte) error {
	log.Info("[IKE][EAP][Nak] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][EAP][Nak] unmarshal(): Payload length %d bytes", len(rawData))
...
func (eapExpanded *EAPExpanded) marshal() ([]byte, error) {
	log.Info("[IKE][EAP][Expanded] marshal(): Start marshalling")
	...
	if len(eapExpanded.VendorData) == 0 {
		log.Warn("[IKE][EAP][Expanded] marshal(): EAP vendor data field is empty")
...
func (eapExpanded *EAPExpanded) unmarshal(rawData []byte) error {
	log.Info("[IKE][EAP][Expanded] unmarshal(): Start unmarshalling received bytes")
	log.Tracef("[IKE][EAP][Expanded] unmarshal(): Payload length %d bytes", len(rawData))
