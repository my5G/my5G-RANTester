...
import (
	...
	ngap_message "my5G-RANTester/internal/control_test_engine/non3gppue/ngap/message"
...
func HandleIKESAINIT(udpConn *net.UDPConn, n3iwfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	log.Infoln("[IKE] Handle IKE_SA_INIT")
	...
	if message == nil {
		log.Error("[IKE] IKE Message is nil")
		return
	}
	...
	if majorVersion > 2 {
		log.Warn("[IKE] Received an IKE message with higher major version")
		...
		notificationPayload := ike_message.BuildNotification(ike_message.TypeNone,
	...
	for _, ikePayload := range message.IKEPayload {
		...
		default:
			log.Warnf(
				"[IKE] Get IKE payload (type %d) in IKE_SA_INIT message, this payload will not be handled by IKE handler",
				ikePayload.Type())
		}
	}
	...
	if securityAssociation != nil {
		...
		if responseSecurityAssociation == nil {
			log.Warn("[IKE] No proposal chosen")
		...
		log.Error("[IKE] The security association field is nil")
	...
	if keyExcahge != nil {
		...
		if chosenDiffieHellmanGroup != keyExcahge.DiffieHellmanGroup {
			log.Warn("[IKE] The Diffie-Hellman group defined in key exchange payload not matches the one in chosen proposal")
			...
			notificationPayload :=
				ike_message.BuildNotification(ike_message.TypeNone, ike_message.INVALID_KE_PAYLOAD, nil, notificationData)
		...
		log.Error("[IKE] The key exchange field is nil")
		// TODO: send error message to UE
		return
	}
	...
	if nonce != nil {
		...
		log.Error("[IKE] The nonce field is nil")
		// TODO: send error message to UE
		return
	}
	...
	if err := GenerateKeyForIKESA(ikeSecurityAssociation); err != nil {
		log.Errorf("Generate key for IKE SA failed: %+v", err)
		return
	}

	// Send response to UE
	responseIKEMessage = ike_message.BuildIKEHeader(ikeSecurityAssociation.RemoteSPI, ikeSecurityAssociation.LocalSPI,
		ike_message.IKE_SA_INIT, ike_message.ResponseBitCheck, message.MessageID)
	...
	if err != nil {
		log.Errorln(err)
		log.Error("[IKE] Encode message failed.")
		return
	}
	...
	if err != nil {
		log.Errorln(err)
		log.Error("[IKE] Encoding IKE message failed")
		return
	}
	...
	if err != nil {
		log.Errorln(err)
		log.Error("[IKE] Encode IKE payload failed.")
		return
	}
...
func HandleIKEAUTH(udpConn *net.UDPConn, n3iwfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	log.Infoln("[IKE] Handle IKE_AUTH")
	...
	if majorVersion > 2 {
		...
		responseNotification :=
			ike_message.BuildNotification(ike_message.TypeNone, ike_message.INVALID_MAJOR_VERSION, nil, nil)
	...
	if !ok {
		...
		responseIKEMessage = ike_message.BuildIKEHeader(message.InitiatorSPI, 0, ike_message.INFORMATIONAL,
	...
	if err != nil {
		log.Errorf("[IKE] Decrypt IKE message failed: %+v", err)
		return
	}
	...
	for _, ikePayload := range decryptedIKEPayload {
		...
		default:
			log.Warnf(
				"[IKE] Get IKE payload (type %d) in IKE_AUTH message, this payload will not be handled by IKE handler",
				ikePayload.Type())
		}
	}
	...
	case PreSignalling:
		...
		if initiatorID != nil {
			log.Info("Ecoding initiator for later IKE authentication")
			...
			if err != nil {
				log.Errorln(err)
				log.Error("[IKE] Encoding ID payload message failed.")
				return
			}
			pseudorandomFunction, ok := NewPseudorandomFunction(ikeSecurityAssociation.SK_pr,
			...
			log.Error("[IKE] The initiator identification field is nil")
			// TODO: send error message to UE
			return
		}
		...
		if certificateRequest != nil {
			log.Info("UE request N3IWF certificate")
			if CompareRootCertificate(certificateRequest.CertificateEncoding, certificateRequest.CertificationAuthority) {
				responseCertificate =
					ike_message.BuildCertificate(ike_message.X509CertificateSignature, n3iwfSelf.N3IWFCertificate)
			}
		}

		if certificate != nil {
			log.Info("UE send its certficate")
			ikeSecurityAssociation.InitiatorCertificate = certificate
		}

		if securityAssociation != nil {
			log.Info("Parsing security association")
		...
		if trafficSelectorInitiator != nil {
			log.Info("Received traffic selector initiator from UE")
			ikeSecurityAssociation.TrafficSelectorInitiator = trafficSelectorInitiator
		} else {
			log.Error("[IKE] The initiator traffic selector field is nil")
			// TODO: send error message to UE
			return
		}
		...
		for {
			...
			if err != nil {
				log.Errorf("[IKE] Random number failed: %+v", err)
				return
			}
	...
	case EAPSignalling:
		...
		if eap != nil {
			if eap.Code != ike_message.EAPCodeResponse {
				log.Error("[IKE][EAP] Received an EAP payload with code other than response. Drop the payload.")
				return
			}
			if eap.Identifier != ikeSecurityAssociation.LastEAPIdentifier {
				log.Error("[IKE][EAP] Received an EAP payload with unmatched identifier. Drop the payload.")
				return
			}
			...
			default:
				log.Errorf("[IKE][EAP] Received EAP packet with type other than EAP expanded type: %d", eapTypeData.Type())
				return
			}

			if eapExpanded.VendorID != ike_message.VendorID3GPP {
				log.Error("[IKE] The peer sent EAP expended packet with wrong vendor ID. Drop the packet.")
				return
			}
			if eapExpanded.VendorType != ike_message.VendorTypeEAP5G {
				log.Error("[IKE] The peer sent EAP expanded packet with wrong vendor type. Drop the packet.")
				return
			}

			eap5GMessageID, anParameters, nasPDU, err := UnmarshalEAP5GData(eapExpanded.VendorData)
			if err != nil {
				log.Errorf("[IKE] Unmarshalling EAP-5G packet failed: %+v", err)
				return
			}
			...
			if eap5GMessageID == ike_message.EAP5GType5GStop {
				...
				identifier, err := GenerateRandomUint8()
				if err != nil {
					log.Errorf("[IKE] Generate random uint8 failed: %+v", err)
					return
				}
			...
			if anParameters != nil {
				...
				if selectedAMF == nil {
					log.Warn("[IKE] No avalible AMF for this UE")
					return
				}
	...
	case PostSignalling:
		...
		if authentication != nil {
			...
			log.Tracef("Expected Authentication Data:\n%s", hex.Dump(expectedAuthenticationData))
			...
			/*
				if !bytes.Equal(authentication.AuthenticationData, expectedAuthenticationData) {
					log.Warn("[IKE] Peer authentication failed.")
		...
		if configuration != nil {
			log.Tracef("[IKE] Received configuration payload with type: %d", configuration.ConfigurationType)
			...
			for _, attribute = range configuration.ConfigurationAttribute {
				...
				case ike_message.INTERNAL_IP4_ADDRESS:
					...
					if len(attribute.Value) != 0 {
						log.Tracef("[IKE] Got client requested address: %d.%d.%d.%d",
							attribute.Value[0], attribute.Value[1], attribute.Value[2], attribute.Value[3])
					}
				default:
					log.Warnf("[IKE] Receive other type of configuration request: %d", attribute.Type)
				}
			}
		} else {
			log.Warn("[IKE] Configuration is nil. UE did not sent any configuration request.")
		}
		...
		if addrRequest {
			...
			attributes = append(attributes, ike_message.BuildConfigurationAttribute(
				ike_message.INTERNAL_IP4_NETMASK, n3iwfSelf.Subnet.Mask))

			thisUE.IPSecInnerIP = ueIPAddr.String()
			log.Tracef("ueIPAddr: %+v", ueIPAddr)

			// Prepare individual traffic selectors
			individualTrafficSelectorInitiator :=
				ike_message.BuildIndividualTrafficSelector(ike_message.TS_IPV4_ADDR_RANGE, ike_message.IPProtocolAll,
					0, 65535, ueIPAddr.To4(), ueIPAddr.To4())
			individualTrafficSelectorResponder :=
				ike_message.BuildIndividualTrafficSelector(ike_message.TS_IPV4_ADDR_RANGE, ike_message.IPProtocolAll,
					0, 65535, n3iwfIPAddr.To4(), n3iwfIPAddr.To4())

			responseTrafficSelectorInitiator = ike_message.BuildTrafficSelectorInitiator(
				[]*ike_message.IndividualTrafficSelector{individualTrafficSelectorInitiator})
			responseTrafficSelectorResponder = ike_message.BuildTrafficSelectorResponder(
				[]*ike_message.IndividualTrafficSelector{individualTrafficSelectorResponder})
			...
			log.Error("[IKE] UE did not send any configuration request for its IP address.")
			return
		}
		...
		if err != nil {
			log.Errorf("[IKE] Create child security association context failed: %+v", err)
			return
		}
		...
		if err != nil {
			log.Errorf("[IKE] Parse IP address to child security association failed: %+v", err)
			return
		}
		// Select TCP traffic
		...
		if errGen := GenerateKeyForChildSA(ikeSecurityAssociation, childSecurityAssociationContext); errGen != nil {
			log.Errorf("[IKE] Generate key for child SA failed: %+v", errGen)
			return
		}
		...
		// Authentication
		responseAuthentication = ike_message.BuildAuthentication(
		...
		if errEncrypt := EncryptProcedure(ikeSecurityAssociation, ikePayload, responseIKEMessage); errEncrypt != nil {
			log.Errorf("Encrypting IKE message failed: %+v", errEncrypt)
			return
		}
		...
		if err = ApplyXFRMRule(false, childSecurityAssociationContext); err != nil {
			log.Errorf("[IKE] Applying XFRM rules failed: %+v", err)
			return
		}
		...
		if thisUE.TemporaryPDUSessionSetupData != nil {
			for {
				if len(thisUE.TemporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
					...
					encryptionTransform := ike_message.BuildTransform(ike_message.TypeEncryptionAlgorithm,
					...
					// ESN transform
					esnTransform :=
						ike_message.BuildTransform(ike_message.TypeExtendedSequenceNumbers, ike_message.ESN_NO, nil, nil, nil)
					...
					trafficSelectorInitiator :=
						ike_message.BuildTrafficSelectorInitiator([]*ike_message.IndividualTrafficSelector{individualTrafficSelector})
					...
					trafficSelectorResponder :=
						ike_message.BuildTrafficSelectorResponder([]*ike_message.IndividualTrafficSelector{individualTrafficSelector})
...
func HandleCREATECHILDSA(udpConn *net.UDPConn, n3iwfAddr, ueAddr *net.UDPAddr, message *ike_message.IKEMessage) {
	log.Infoln("[IKE] Handle CREATE_CHILD_SA")
	...
	thisUE := ikeSecurityAssociation.ThisUE
	if thisUE == nil {
		log.Error("UE context is nil")
		return
	}
	// PDU session information
	if thisUE.TemporaryPDUSessionSetupData == nil {
		log.Error("No PDU session information")
		return
	}
	...
	if trafficSelectorInitiator == nil {
		log.Error("[IKE] The traffic selector initiator field is nil")
		return
	}

	if trafficSelectorResponder == nil {
		log.Error("[IKE] The traffic selector responder field is nil")
		return
	}
	...
	if err != nil {
		log.Errorf("Build PDU session resource setup response transfer failed: %+v", err)
		return
	}
	...
	for {
		if len(temporaryPDUSessionSetupData.UnactivatedPDUSession) != 0 {
			...
			if ok := ike_message.AppendTransformToProposal(proposal, encryptionTransform); !ok {
				log.Error("Generate IKE message failed: Cannot append to proposal")
				temporaryPDUSessionSetupData.UnactivatedPDUSession =
			...
			// Integrity transform
			if pduSession.SecurityIntegrity {
				integrityTransform := ike_message.BuildTransform(ike_message.TypeIntegrityAlgorithm,
			...
			// ESN transform
			esnTransform := ike_message.BuildTransform(ike_message.TypeExtendedSequenceNumbers,
...
func parseIPAddressInformationToChildSecurityAssociation(
	...
	log.Tracef("Local TS: %+v", trafficSelectorLocal.StartAddress)
	log.Tracef("Remote TS: %+v", trafficSelectorRemote.StartAddress)
